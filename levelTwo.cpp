...
bool levelTwo::init() {
	...
	//background
	auto background = Sprite::create("levelTwo.png");
    ...
    listener2->onMouseDown = [this](Event* event) {
        ...
        if (numBullets == 0) {
            ...
            auto scoreScene = levelTwoSB::createScene(score, achieved, total);
    ...
    this->schedule([this, green, red](float dt) {
        ...
        if (nextDuckIndex < ducks.size()) {
            ...
            CCLOG("No more ducks to add");
            auto scoreScene = levelTwoSB::createScene(score, achieved, ducks.size());
            Director::getInstance()->replaceScene(TransitionFade::create(1.0f, scoreScene));
            return;
        }
