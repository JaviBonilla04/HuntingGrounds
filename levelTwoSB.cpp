...
levelTwoSB::levelTwoSB(int score, int achieved, int total)
    : score(score), achieved(achieved), total(total) {}

levelTwoSB::~levelTwoSB() {}

levelTwoSB* levelTwoSB::createScene(int score, int achieved, int total)
{
    levelTwoSB* scene = new levelTwoSB(score, achieved, total);
...
bool levelTwoSB::init(int score, int achieved, int total)
{
    if (!Scene::init())
    {
        return false;
    ...
    auto layerCero = Sprite::create("levelTwoScoreBoard.png");
...
void levelTwoSB::toMenu(Ref* send)
{
