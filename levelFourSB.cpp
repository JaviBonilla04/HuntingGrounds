...
levelFourSB::levelFourSB(int score, int achieved, int total)
    : score(score), achieved(achieved), total(total) {}

levelFourSB::~levelFourSB() {}

levelFourSB* levelFourSB::createScene(int score, int achieved, int total)
{
    levelFourSB* scene = new levelFourSB(score, achieved, total);
...
bool levelFourSB::init(int score, int achieved, int total)
{
    if (!Scene::init())
    {
        return false;
    ...
    auto layerCero = Sprite::create("levelFourScoreBoard.png");
...
void levelFourSB::toMenu(Ref* send)
{
