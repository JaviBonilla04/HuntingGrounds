...
levelOneSB::levelOneSB(int score, int achieved, int total)
    : score(score), achieved(achieved), total(total) {}

levelOneSB::~levelOneSB() {}

levelOneSB* levelOneSB::createScene(int score, int achieved, int total)
{
    levelOneSB* scene = new levelOneSB(score, achieved, total);
...
bool levelOneSB::init(int score, int achieved, int total)
{
    if (!Scene::init())
    {
        return false;
    ...
    auto layerCero = Sprite::create("levelOneScoreBoard.png");
...
void levelOneSB::toMenu(Ref* send)
{
